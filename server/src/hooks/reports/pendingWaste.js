...
const addToWasteList = ({
  ...
  const endTime = moment(turn.endTime);
  const ed = {
    ...getDateJSON({
      date,
    }),
...
module.exports = function () {
  return async (context) => {
    ...
    const productionOrderStatusesService = app.service(
    ...
    if (
      (get(params, "$pendingWaste") || get(params, "query.$pendingWaste")) &&
      method === "find"
    ) {
      params.$pendingWaste =
        get(params, "$pendingWaste") || get(params, "query.$pendingWaste");
    ...
    if (params.$pendingWaste && type === "after") {
      ...
      const { data: productionOrders } = await productionOrdersService.find({
        query: {
          ...
          $or: [
            { actualEndDate: { $gte: startD.toDate() } },
            { actualEndDate: null },
          ],
        },
      });
      ...
      const { data: products } = await productsService.find({
        query: {
          id: {
            $in: lodash.map(productionOrders, ({ productId }) => productId),
          },
        },
      });
      const productsMap = lodash.keyBy(products, "id");
      const {
        data: productionOrderStatuses,
      } = await productionOrderStatusesService.find({
        query: {
          id: {
            $in: lodash.map(
              productionOrders,
      ...
      const productionOrderStatusesMap = lodash.keyBy(
      ...
      await Promise.all(
        lodash.map(machines, async (machine) => {
          ...
          await Promise.all(
            lodash.map(
              get(machineProductionOrdersMap, get(machine, "id")),
              async (itemOP) => {
                const productionOrder = get(itemOP, "dataValues") || itemOP;
                productionOrder.machine = machine;
                productionOrder.mold = get(
                  moldsMap,
                  get(productionOrder, "moldId")
                );
                productionOrder.product = get(
                  productsMap,
                  get(productionOrder, "productId")
                );
                productionOrder.production_order_status = get(
                  productionOrderStatusesMap,
                  get(productionOrder, "productionOrderStatusId")
                );
                const actualStartOPD = moment(
                  get(productionOrder, "actualStartDate")
                ).startOf("day");
                const startOPD = actualStartOPD.isAfter(startD)
                  ? actualStartOPD
                  : startD;
                const plant =
                ...
                if (get(plant, "qualityTrackFrequency") === "Diario") {
                  ...
                  getProduction = false;
                  const actualEndOPD = moment(
                    get(productionOrder, "actualEndDate")
                  ).startOf("day");
                  const sd = {
                    ...getDateJSON({
                  ...
                  wasteList.push({
                    id: `${get(productionOrder, "id")}${moment(
                      sd
                    ).toISOString()}`,
                    poi: productionOrder,
                    plant,
                ...
                wasteList = lodash.filter(wasteList, (item, index) => {
                  ...
                  if (showHistory && wItem) {
                    ...
                    const weight =
                      parseFloat(get(item, "poi.product.weight")) || 0;
                    item._id = wItem._id;
                    ...
                    item.cp = wItem.cp;
                    item.cpw =
                      Math.round(((wItem.cp * weight) / 1000) * 100) / 100;
                    item.wp = wItem.wp;
                    item.wpw =
                      Math.round(((wItem.wp * weight) / 1000) * 100) / 100;
                    item.w = weight;
                    item.wji = wItem.wji;
